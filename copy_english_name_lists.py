...
for filename in os.listdir('localisation/english/name_lists')[:]:
	sourceFile = open('localisation/english/name_lists/' + filename, 'r')
	for target in targetLanguages:
		...
		if os.path.isfile('localisation/' + target.replace("l_","") + '/name_lists/' + newFilename) and not overwrite: continue
		targetFile = open('localisation/' + target.replace("l_","") + '/name_lists/' + newFilename, 'w+')
